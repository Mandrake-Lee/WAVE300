...
/* $Id: mtlkstartup.h 11870 2011-10-31 18:18:52Z fleytman $ */
...
static __INLINE
void MTLK_STEPS_INFO_S(const char *str)
{
  ILOG4_S("%s", str);
}

static __INLINE
void MTLK_STEPS_INFO_SD(const char *str, int var_01)
{
  ILOG4_SD("%s (%d)", str, var_01);
}

static __INLINE
void MTLK_STEPS_INFO_SDD(const char *str, int var_01, int var_02)
{
  ILOG4_SDD("%s (%d) (%d)", str, var_01, var_02);
}

static __INLINE
void MTLK_STEPS_WARNING_S(const char *str)
{
  WLOG_S("%s", str);
}

static __INLINE
void MTLK_STEPS_ERROR_S(const char *str)
{
  ELOG_S("%s", str);
}

static __INLINE
void MTLK_STEPS_ERROR_SD(const char *str, int var_01)
{
  ELOG_SD("%s (%d)", str, var_01);
}
...
#define _MTLK_FWD_STEP(exec_cond, module_name, step_name, object, success_op, iter_ctr, step_func,                        \
  ...
  {                                                                                                                       \
    ...
    __macro_mix_guard_fwd_##suffix = __macro_mix_guard_fwd_##suffix;                                                      \
    __object_mix_guard_fwd_##module_name = __object_mix_guard_fwd_##module_name;                                          \
    ++g_CurrentStepNumber;                                                                                                \
    ...
    if(exec_cond) {                                                                                                       \
      ...
      if (success_cond) {                                                                                                 \
        ...
        __res = (error_val);                                                                                              \
        MTLK_STEPS_ERROR_SD(STEPS_PFX #module_name ":" #step_name "_" #suffix " FAILED! step" , g_CurrentStepNumber);     \
        goto __ON_FWD_ERROR__;                                                                                            \
      }                                                                                                                   \
    } else {                                                                                                              \
      __res = MTLK_ERR_OK;                                                                                                \
...
#define _MTLK_FWD_STEPS_RETURN(module_name, object, suffix, rollback_func, rollback_params)                             \
      __macro_mix_guard_bwd_##suffix = __macro_mix_guard_bwd_##suffix;                                                  \
      __object_mix_guard_bwd_##module_name = __object_mix_guard_bwd_##module_name;                                      \
    }                                                                                                                   \
    __macro_mix_guard_fwd_##suffix = __macro_mix_guard_fwd_##suffix;                                                    \
    __object_mix_guard_fwd_##module_name = __object_mix_guard_fwd_##module_name;                                        \
