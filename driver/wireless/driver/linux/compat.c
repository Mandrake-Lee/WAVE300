/*
 * $Id: compat.c 11067 2011-04-27 12:45:53Z grygoriis $
 *
...
static int load_firmware_realloc_buffer(FIRMWARE_DATA *fw_priv, int min_size)
{
  ...
  if(fw_priv->fw.data) {
    memcpy(new_data, fw_priv->fw.data, fw_priv->count);
...
static int load_firmware_thread(void *arg)
{
  ...
  sprintf(temp, "/bin/cat "MTLK_FIRMWARE_PATH"%s > /proc/"MTLK_FIRMWARE_PROC_NAME, (char *)arg);
  argv[2] = temp;
