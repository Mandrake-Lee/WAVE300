...
#else /* MTCFG_TXMM_HISTORY_LENGTH */
...
static int _mtlk_txmm_del_buf_from_que(mtlk_txmm_base_t* obj, mtlk_txmm_block_e que_idx, mtlk_txmm_block_t* buf, int preserve_order)
{
  ...
  if (que_idx < SIZEOF(obj->ques))
  {
    ...
    {
      ELOG_DPDD("deletion of invalid block "
           "from queue#%d (%p, %d, %d)", 
           que_idx, buf, buf->que, (int)buf->idx);
    }
  }
...
static void _mtlk_txmm_send_block_prepare(mtlk_txmm_base_t* obj, mtlk_txmm_block_t* block, PMSG_OBJ pmsg)
{
  ...
  memcpy(payload_buff, block->data.payload, block->data.payload_size);
...
static __INLINE int _mtlk_txmm_on_cfm(mtlk_txmm_base_t* obj, PMSG_OBJ pmsg)
{
  ...
  if (block)
  {
    ...
    memcpy(block->data.payload, payload, obj->cfg.max_payload_size);
...
static void _mtlk_txmm_attach_data(mtlk_txmm_t *obj, mtlk_txmm_msg_t *msg)
{
  mtlk_txmm_block_t* block = NULL;
                               ...
                               uint32            timeout_ms)
{
  ...
  block->vap_handle = obj->vap_handle;
...
static void _mtlk_txmm_free(mtlk_txmm_base_t* obj, mtlk_txmm_data_t* data)
{
  mtlk_txmm_block_t* block = _mtlk_txmm_block_by_data(data);
...
mtlk_txmm_msg_get_empty_data (mtlk_txmm_msg_t *msg, mtlk_txmm_t *obj)
{
  ...
  if (msg_obj == NULL) {
    ...
    _mtlk_txmm_attach_data(obj, msg);
...
mtlk_txmm_msg_cancel (mtlk_txmm_msg_t *msg)
{
  ...
  if (obj) {
    ...
    if (msg->data) {
      ...
      _mtlk_txmm_free(msg->obj->base, msg->data);
