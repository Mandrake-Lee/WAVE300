...
if logger_preprocessing_enabled && \
  ...
  # Create logmacros.[ch] and loggroups.h because source code depends on them
  for file in $abs_builddir/logmacros.h $abs_builddir/logmacros.c $abs_builddir/loggroups.h; do
  ...
  for file in $DEPS; do
    ...
    if test ! -e $TIMESTAMP_FILE -o $file -nt $TIMESTAMP_FILE; then
      ...
      logpreprocessed_file=$abs_builddir/`basename $file`_$$.lgprpd
      perl $abs_tools_srcdir/rtlogger/logprep/logprep.pl gcc \
                                                         $logger_origin_id $logger_origin_name \
                                                         $abs_builddir/loggroups.h $abs_builddir/logmacros.h \
                                                         $abs_builddir/logmacros.c $abs_builddir/$logger_origin_name.scd \
                                                         < $file > $logpreprocessed_file || { echo Logprep error while processing file $file && exit 1; }
      #Do not rebuild all sources because of loggger database changes
      touch -m -t 198001010000.00 $abs_builddir/loggroups.h $abs_builddir/logmacros.c $abs_builddir/logmacros.h
      #Do rebuild logmacro_database.o because it could be new macros were added
      rm -f $abs_builddir/logmacro_database.o

      if diff -q $file $logpreprocessed_file >/dev/null 2>&1; then
        rm -f $logpreprocessed_file
      else
        mv $logpreprocessed_file $file
      fi
