...
#define SAFE_LEN(x) (strlen(x) > MTLK_LED_STRING_BUFFER_SIZE ? (MTLK_LED_STRING_BUFFER_SIZE-1) : strlen(x))
...
char baseb_led[MTLK_SECURITY_LEDS_INDEX_MIN];
/*wep*/
int    wep_start = 0;
...
static char link_status_script_path[MTLK_LED_STRING_BUFFER_SIZE]=""; 
static char   link_script_path[MTLK_LED_STRING_BUFFER_SIZE]="";      
...
int leds_ctrl_set_link_path(char* str)
{
  ...
  if (size <= sizeof(link_script_path)) {
    memcpy(link_script_path, str, size);
    ILOG2_S("leds API: link_script_path = %s",link_script_path);
    return MTLK_ERR_OK;
  }
...
int leds_ctrl_set_link_status_path(char* str)
{
  ...
  if (size <= sizeof(link_status_script_path)) {
    memcpy(link_status_script_path, str, size);
    ILOG2_S("leds API: link_status_script_path = %s",link_status_script_path);
    return MTLK_ERR_OK;
  }
...
dataex_report_error(int ioctl_rslt, char *pdata)
{
  ...
  case WE_GEN_DATAEX_PROTO_MISMATCH:
    ELOG_DD("Protocol version mismatch (got %u, expected %u)",
...
*   There are two options to write leds:
*     baseb0 - write value to MAC. The MAC write to LED
*     path to proc - the value is written to proc by driver.
*   The null indicates that no need to write to Link LED.
*/
void
leds_control(char * val, WE_GEN_DATAEX_STATUS* pstatus)
{
  ...
  WE_GEN_DATAEX_LED *pleds_to_mac;
  ...
  if (!(cfg_mask & DHFLAG_NO_WRITE_LED) &&
    ...
    for (i = 0; i < MTLK_SECURITY_LEDS_INDEX_MIN; i++) {
      ...
      if (memcmp(leds_config[i].file, "null", SAFE_SIZEOF("null")-1) == 0) {
      ...
      else if (memcmp(leds_config[i].file, "baseb", SAFE_SIZEOF("baseb")-1) == 0) {
        /**************************************
        * basebx - write value to MAC if band supported
        */
        ILOG2_DD("baseb%d, leds_config[i].index = %d",baseb_led[i], leds_config[i].index);
        if (val[0] == wlan_link_off[0] || pstatus->frequency_band == leds_config[i].index) {
          /*active band (or disconnected), can write (only one band can be supported)*/
          pdata = (char *) calloc(1, MTLK_IFCMD_DATALEN * sizeof(char)); //TODO: smaller data size
          if (!pdata) {
            ELOG_V("Out of memory");
            goto stop;
          }
          if (baseb_led[i] == CHAR_MAX) {
            ELOG_V("missmatch baseb_led param value");
            goto stop;
          }
          preq = (WE_GEN_DATAEX_REQUEST *) pdata;
          preq->ver = WE_GEN_DATAEX_PROTO_VER;
          preq->cmd_id = WE_GEN_DATAEX_CMD_LEDS_MAC;
          preq->datalen = MTLK_IFCMD_DATALEN;

          pleds_to_mac = (WE_GEN_DATAEX_LED *)(preq+1);
          pleds_to_mac->u8BasebLed = baseb_led[i];
          /*TODO: char to binary - ??? needed?*/
          pleds_to_mac->u8LedStatus = *val - '0';

          ioctl_rslt = driver_ioctl(MTLK_IOCTL_DATAEX, pdata, MTLK_IFCMD_DATALEN);

          if (ioctl_rslt == -1) {
            WLOG_V("Can't write value to MAC\n");
            goto stop;
          }
        ...
        */
        ...
        if (pstatus->frequency_band == leds_config[i].index) {
          ...
          /*TODO: SAFE_LEN ??*/
          if (fwrite(val, 1, strlen(val), fp) < strlen(val))
                      ...
                      ,int *do_leds)
{
  ...
  switch (wps_status_mirror) {
    case MTLK_WPS_IDLE:
      ...
      strcpy(security_leds_status[wps_error_led], security_off);
      strcpy(security_leds_status[wps_activity_led], security_off);
    ...
    case MTLK_WPS_REGISTER:
      ...
      error_start = 0;
      strcpy(security_leds_status[wps_error_led], security_off);
      strcpy(security_leds_status[wps_activity_led], wps_in_process);
      ...
      */
      if (psecurity_status->wep_enabled == 0 && psecurity_status->security_on) {
        ...
        if (wps_activity_led == security_led)
          strcpy(security_leds_status[wps_activity_led], security_on);
        else
          strcpy(security_leds_status[wps_activity_led], security_off);
      ...
      if (error_start == 0) {
        ...
        if (wps_status_mirror != wps_status_before) {
          ...
          */
          ...
          strcpy(security_leds_status[wps_activity_led], wps_errors[wps_status_mirror]);
...
* set leds_config[].file and baseb_led[] with the proper string and value.
...
*
* Options:
* WLAN_LED_2_4     - null, basebx, /dev/gpiox
* WLAN_LED_5_2     - null, basebx, /dev/gpiox
...
int leds_parse_configs(void) 
{
  ...
  while ((i < MTLK_SECURITY_LEDS_MAX) && (open_count < MTLK_SECURITY_LEDS_MAX)) {
    ...
    while ( (fgets(line, MTLK_STRING_BUFFER_LINE_SIZE, fp) != NULL) &&
      ...
      if (ptr) {
        /*we have a match*/
        ptr= strchr(line, '=') + MTLK_CHAR_OFFSET_HW_VAL;
        /*copy to array*/
        strncpy(leds_config[i].file, ptr, SAFE_LEN(ptr)-MTLK_NEW_LINE_ADDITION);
        ILOG2_SD("set file with %s, strlen(ptr) = %d"
                 ,leds_config[i].file, SAFE_LEN(ptr)-1);
#ifdef MTLK_DEBUG_CHARS
        for (j=0; j<13; j++)
          ILOG0_D("char = 0x%x", leds_config[i].file[j]);
#endif
        /*set baseb_led with value. use leds_config[i].index to sync between baseb_led to the entry in leds_config
         database (i can be higher in one loop, lowe at the later one, use leds_config[i].index always true)*/
        if (strncmp("baseb", ptr, (SAFE_LEN(ptr)-MTLK_NEW_LINE_ADDITION-MTLK_LED_ADDITION)) == 0) {
          strncpy(&baseb_led[leds_config[i].index], ptr+(SAFE_LEN(ptr)-MTLK_NEW_LINE_ADDITION-MTLK_LED_ADDITION), 1);
          ILOG2_DD("baseb_led[%d] = 0x%x",leds_config[i].index,baseb_led[leds_config[i].index]);
          if (baseb_led[leds_config[i].index] >= '0')
            baseb_led[leds_config[i].index] -= '0';
        }
        i++;
      }
      else {
  ...
  for (;;) {
    ...
    */
    ...
    if (presp->status == WE_GEN_DATAEX_SUCCESS) {
      memcpy(&led_status, (WE_GEN_DATAEX_STATUS*)(presp+ 1),
