...
static char wps_script_path[MTLK_LED_STRING_BUFFER_SIZE]         = "";
...
int wps_ctrl_set_link_status_path(char* str)
{
  ...
  if (size <= sizeof(link_status_script_path)) {
    ...
    ILOG2_S("wps API: link_status_script_path = %s",link_status_script_path);
    return MTLK_ERR_OK;
  }
...
int wps_ctrl_set_wps_script_path(char* str)
{
  ...
  if (size <= sizeof(wps_script_path)) {
    memcpy(wps_script_path, str, size);
    ILOG2_S("wps API: wps_script_path = %s",wps_script_path);
    return MTLK_ERR_OK;
  }
