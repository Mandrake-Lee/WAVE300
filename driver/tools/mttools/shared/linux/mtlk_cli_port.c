...
static int
_mtlk_cli_port_get_fname (const char *name, char *fname, int size)
{
  int res = snprintf(fname, size, MTLK_CLI_PORT_FNAME_FMT, name);
  return (res >= size)?MTLK_ERR_BUF_TOO_SMALL:MTLK_ERR_OK;
}
