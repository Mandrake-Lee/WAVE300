...
static int  _mtlk_cli_peer_init(mtlk_cli_peer_t *peer);
static int  _mtlk_cli_peer_start(mtlk_cli_peer_t *peer, SOCKET sd, mtlk_cli_srv_t *srv);
...
_mtlk_cli_srv_main_thread_proc (mtlk_handle_t context)
{
  ...
  while (1) {
    ...
    res = _mtlk_cli_peer_start(peer, sd, srv);
    if (res != MTLK_ERR_OK) {
      ELOG_D("Can't initialize peer (err=%d)", res);
      goto loop;
    }
...
mtlk_cli_srv_start (mtlk_cli_srv_t *srv, uint16 port)
{
  ...
  sprintf(buf, "%u", (uint32)port);

  memset(&hints, 0, sizeof(hints));
#ifdef MTCFG_IPV4_ONLY_CLI
                               ...
                               mtlk_handle_t          ctx)
{
  ...
  memcpy(srv_clb->cmd_name, cmd_name, size);
...
static int
_mtlk_cli_peer_start (mtlk_cli_peer_t *peer, SOCKET sd, mtlk_cli_srv_t *srv)
{
  peer->sd             = sd;
  peer->srv            = srv;
  peer->thread_stop    = 0;
...
static int
_mtlk_cli_peer_init (mtlk_cli_peer_t *peer)
{
