...
mtlk_irbd_root_init (void)
{
  ...
  mtlk_osal_atomic_set(&drv_data->minor_cnt, 0);
                 ...
                 const char  *unique_desc)
{
  ...
  int              minor = (int)mtlk_osal_atomic_inc(&mtlk_irbd_drv.minor_cnt) - 1;
  void            *buf   = NULL;
  int              len   = strlen(unique_desc);
  ...
  MTLK_INIT_TRY(irbd, MTLK_OBJ_PTR(irbd))
    ...
    memcpy(irbd->unique_desc, unique_desc, len + 1);

    MTLK_INIT_STEP_EX(irbd, IRBD_CREATE_CDEV_NODE, MTLK_OBJ_PTR(irbd), 
                      mtlk_cdev_node_create, (&mtlk_irbd_drv.cdev, minor, _mtlk_irbd_ioctl_handler, HANDLE_T(irbd)),
                      irbd->cdev_node,
                      irbd->cdev_node != NULL,
                      MTLK_ERR_UNKNOWN);
...
{
  mtlk_handle_t h = _mtlk_irb_hash_register(&irbd->hash,
                                            evts,
                                            nof_evts,
                                            handler,
                                            context);
                      ...
                      uint32             size)
{
  ...
  memcpy(msg_buf + sizeof(*hdr), buffer, size);
...
_mtlk_irbd_print_irbd_topology (struct seq_file *s, mtlk_irbd_t *irbd)
{
  ...
  mtlk_dlist_foreach(&irbd->sons, e, head) {
    mtlk_irbd_t* son = 
      MTLK_CONTAINER_OF(e, mtlk_irbd_t, lentry);
