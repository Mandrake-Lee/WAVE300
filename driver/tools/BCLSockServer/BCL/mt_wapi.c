...
MT_UBYTE ReadFileParmterToReadfrom_WL()
{
    ...
    if (!fpout)
    {
        error("could not open '"__FTP_PATH__"/dataread.txt' for reading");
        return 0;
    }
    ...
    while ((read = fscanf(fp,"%08lx %08lx %08lx",&chip_mode,&Add_val,&len)) == 3) 
    {
        ...
        /*Write to file the chip add and len*/
        fwrite(&chip_mode, 4, 1, fpout);
        fwrite(&Add_val, 4, 1, fpout);
        fwrite(&len, 4, 1, fpout);
        /* Translate addresses */
        ...
        while (len) 
        {
            if(len > HYP_MAX_MSG_SIZE) 
            {
                val_countread=HYP_MAX_MSG_SIZE;
                HypPciRead(HYP_MEMORY, Add_val,HYP_MAX_MSG_SIZE, buffer);
                len=len-HYP_MAX_MSG_SIZE;
                Add_val += HYP_MAX_MSG_SIZE * 4;
            } else 
            {
                val_countread=len;
                HypPciRead(HYP_MEMORY, Add_val,(MT_UBYTE)len, buffer);
                len=0;
            }
            
            fwrite(buffer, 4, val_countread, fpout);
	...
	if (read != EOF)
    {
        mt_print(2,"failed to read paramters from 'dataread.txt'\n");
        error("failed to read paramters from 'dataread.txt'");
        return MT_RET_FAIL;
    }
...
MT_UINT32 TFTP_Get(MT_BYTE* source_path, MT_BYTE* dest_path)
{
    ...
    if (dest_path[0] != '/') {
	    sprintf(destPath, "%s/%s", __FTP_PATH__, dest_path);
    } else {
	    sprintf(destPath, "%s", dest_path);
    }

    sprintf(cmd, "tftp -g -r \"%s\" -l \"%s\" \"%s\" ", source_path, destPath, ip);
...
MT_UINT32 TFTP_Put(MT_BYTE* source_path, MT_BYTE* dest_path)
{
    ...
    /* Use the /tmp path unless a full path is given */
    if (source_path[0] != '/') {
    	sprintf(srcPath, "%s/%s", __FTP_PATH__, source_path);
    } else {
	sprintf(srcPath, "%s", source_path);
    }
    
    sprintf(cmd, "tftp -p -l \"%s\" -r \"%s\" \"%s\" ", srcPath, dest_path, ip);
    mt_print(2,"Executing: %s\n", cmd);
    nResult = system(cmd);
    if (nResult != 0) {
...
MT_UINT32 Chipvar_Get(MT_UBYTE cpu, MT_UINT32 index, MT_UINT32 offset, MT_UINT32 count)
{
	...
	dmem((MT_UBYTE)chipVar.size,(MT_UINT32)buffer,(MT_UBYTE)count);
