...
drv_process_pkt(cqueue_t *pqueue)
{
  ...
  oid   = LOG_INFO_GET_OID(log_evt.info);
  gid   = LOG_INFO_GET_GID(log_evt.info);
  fid   = LOG_INFO_EX_GET_FID(log_evt.info_ex);
  lid   = LOG_INFO_EX_GET_LID(log_evt.info_ex);
    ...
    case LOG_DT_INT64:
      {
        ...
        if (log_to_syslog) {
          syslog(syslog_pri, "    datatype = LOG_DT_INT64: value = %lld", val);
        }
        if (text_protocol) {
          printed = snprintf(buf, sizeof(buf),
...
drv_process_next_pkt(cqueue_t *pqueue)
{
  ...
  datalen = LOG_INFO_EX_GET_DSIZE(log_evt.info_ex);
