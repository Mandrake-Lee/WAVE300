...
cqueue_get(cqueue_t *pqueue, int offset, int len, unsigned char *data)
{
  ...
  if (offset_begin_idx < pqueue->end_idx) {
    // [XXXX<begin>....<end>XXXX]
    memcpy(data, &pqueue->data[offset_begin_idx], len);
  } else {
    // begin >= end
    // [....<end>XXXX<begin>....]
    ...
    if (to_copy > 0)
      memcpy(data, &pqueue->data[offset_begin_idx], to_copy);
    if (len == to_copy) { // We're copying lesser data than available at the end
      ...
      memcpy(data + to_copy, pqueue->data, bytes_remaining);
    }
  }
}
...
cqueue_push_back (cqueue_t *pqueue, unsigned char *data, int len)
{
  ...
  if (pqueue->begin_idx < pqueue->end_idx) {
    ...
    if (to_write > 0) {
      memcpy(&pqueue->data[pqueue->end_idx], data, to_write);
    }
    if (len == to_write) {
      // Everything was copied
      ...
      memcpy(pqueue->data, data + to_write, bytes_remaining);
    ...
    if (pqueue->is_empty)
      pqueue->is_empty = 0;
    memcpy(&pqueue->data[pqueue->end_idx], data, len);
    pqueue->end_idx += len;
  }
}
