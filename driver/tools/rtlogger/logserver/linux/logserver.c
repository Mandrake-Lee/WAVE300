...
accept_connections(int mother_socket)
{
  ...
  for (;;) {
    ...
    // Reset variables so they aren't freed if error occures on next iteration:
    s = 0;
...
main_loop(int mother_socket)
{
  ...
  while (!terminated) {
    ...
    if (ret < 0) {
      ...
      if (errno == EINTR) {
        // Signal caught, this doesn't neccessarily means termination
        continue;
      } else {
        terminated = 1;
        continue;
    ...
    if (log_cdev && FD_ISSET(log_cdev->fd, &input_set)) {
      if (parse_events) {
        ...
        if (0 >= cdev_read_to_q(&parse_event_q, log_cdev)) {
            ELOG_S("While queueing log data for console: %s", strerror(errno));
        ...
        if (pcon_list) {
          ...
          if (0 >= cdev_read_to_q(&pcon->out_q, log_cdev)) {
            ELOG_S("While queueing log data for [%s]",
              inet_ntoa(pcon->sockaddr.sin_addr));
            log_cdev_error_state = 1;
            close_datasource();
...
scd_load(void)
{
  ...
  for (;;) {
    retval = get_line(scd_filename, buf, ARRAY_SIZE(buf), fl, 1, &eof);
    if (retval != 0) {
      rslt = -1;
      goto cleanup;
    }
...
_print_help (const char *app_name)
{
  ...
  mtlk_argv_print_help(stdout,
                       app_fname,
                       "Metalink Log Server v." MTLK_SOURCE_VERSION,
                       all_params,
...
main(int argc, char *argv[])
{
  ...
  ILOG0_S("Metalink Log Server v. %s", MTLK_SOURCE_VERSION);
