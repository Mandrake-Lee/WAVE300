...
/* to use with %u, %x, %X etc. conversion specifiers as they expect unsigned int argument */
#define _BYTE_ARG_U(b,i)            __BYTE_ARG_TYPE(unsigned int,(b),(i))
#define _WORD_ARG_U(b,ih,il)        __WORD_ARG_TYPE(unsigned int,(b),(ih),(il))
...
#define IP4_PRINTF_ARG(x) \
  _BYTE_ARG_U((x), 3),_BYTE_ARG_U((x), 2),_BYTE_ARG_U((x), 1),_BYTE_ARG_U((x), 0)
