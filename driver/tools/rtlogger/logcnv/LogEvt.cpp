...
CLogEvt::Read (istream &in_s)
{
  ...
  try {
    in_s.read((char *)&log_evt, sizeof(log_evt.info) + 
                                sizeof(log_evt.info_ex) + 
                                sizeof(log_evt.timestamp));
  }
  ...
  if (!LOG_IS_CORRECT_INFO(log_evt.info)) {
    throw exc_bad_evt_hdr(log_evt.info);
  }
    
  /* Get endianness */
  m_reversed = LOG_IS_INVERSED_ENDIAN(log_evt.info);

  /* Reverse log event header fields if required */
  if (m_reversed) {
    log_evt.info      = REVERSE16(log_evt.info);
    log_evt.info_ex   = REVERSE32(log_evt.info_ex);
    log_evt.timestamp = REVERSE32(log_evt.timestamp);
  }

  /* Extract log event information */
  m_oid = (uint8)LOG_INFO_GET_OID(log_evt.info);
  m_gid = (uint8)LOG_INFO_GET_GID(log_evt.info);
  m_fid = (uint16)LOG_INFO_EX_GET_FID(log_evt.info_ex);
  m_lid = (uint16)LOG_INFO_EX_GET_LID(log_evt.info_ex);
  m_wlanif = (uint8)LOG_INFO_EX_GET_WLAN_IF(log_evt.info_ex);
  dsize = LOG_INFO_EX_GET_DSIZE(log_evt.info_ex);
...
CLogEvtFmt::PutParam (const T& val)
{
  ...
  if (m_offset == string::npos) {
    throw exc_bad_format(m_format, m_offset, (uint32)val);
  }
