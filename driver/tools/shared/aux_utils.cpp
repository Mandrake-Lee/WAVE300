...
CStrTokenizer::iterator::iterator(const CStrTokenizer& tokenizer,
...
CStrTokenizer::iterator::operator bool() const
{
    return m_pos <= m_tokenizer.m_string.length();
}
CStrTokenizer::iterator& CStrTokenizer::iterator::operator = (const iterator& other)
{
...
CStrTokenizer::iterator& CStrTokenizer::iterator::operator ++()
{
    iterator temp(m_tokenizer, m_separator, m_pos);
    return (*this = temp);
}
