...
#include <arpa/inet.h>
#define PAIRINGD_SOCKET "/var/bewan/tmp/pairingd.sock"
/*
* Send state message to pairingd
*/
static int notify_result(const char *command)
{
        int     sock;
        struct  sockaddr_un pairingd_addr; /* address of grabber */
        ...
        bzero(&pairingd_addr, sizeof(pairingd_addr));
        pairingd_addr.sun_family = AF_UNIX;
        strcpy(pairingd_addr.sun_path, PAIRINGD_SOCKET);

        sendto(sock, command, strlen(command) + 1, 0, (struct sockaddr *)&pairingd_addr, sizeof(struct sockaddr_un));
...
SM_STATE(WPA_PTK_GROUP, REKEYESTABLISHED)
{
	...
	wpa_auth_vlogger(sm->wpa_auth, sm->addr, LOGGER_INFO,
			 ...
			 sm->wpa == WPA_VERSION_WPA ? "WPA" : "RSN");
#ifdef CONFIG_IBOX	
	char buf[sizeof(MACSTR)];
	char cmd[256];
	memset(cmd, 0, sizeof(cmd));
	memset(buf, 0, sizeof(buf));
	if (sm->addr != NULL) {
		snprintf(buf, sizeof(buf), MACSTR, MAC2STR(sm->addr));
	} else {
		snprintf(buf, sizeof(buf), MACSTR, 0,0,0,0,0,0);
	}
	snprintf(cmd, sizeof(cmd), "%s %s set key done in %s", "wlan0" /* TODO hapd->conf->iface */,
			 buf, sm->wpa == WPA_VERSION_WPA ? "WPA/WPAPSK" : "WPA2/WPA2PSK");
	notify_result(cmd);
#endif	// CONFIG_IBOX
	sm->has_GTK = TRUE;
}
...
SM_STATE(WPA_PTK_GROUP, KEYERROR)
{
	...
	snprintf(cmd, sizeof(cmd), "%s %s WPA/WPA2 bad key", "wlan0" /* TODO hapd->conf->iface */, buf);
	notify_result(cmd);
#endif	// CONFIG_IBOX
}
