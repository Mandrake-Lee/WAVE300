...
static int notify_result(const char *command)
{
	...
	memset(&pairingd_addr, 0, sizeof(pairingd_addr));
			...
			const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (res == HOSTAPD_ACL_REJECT) {
		...
		if (mgmt->sa != NULL) {
			snprintf(buf, sizeof(buf), MACSTR, MAC2STR(mgmt->sa));
		} else {
			snprintf(buf, sizeof(buf), MACSTR, 0,0,0,0,0,0);
		}
		snprintf(cmd, sizeof(cmd), "%s %s is not allowed", hapd->conf->iface, buf);
		notify_result(cmd);
#endif	// CONFIG_IBOX
		goto fail;
	}
			   ...
			   size_t len, int ok)
{
	...
	if (status_code == WLAN_STATUS_SUCCESS &&
		...
		memset(buf, 0, sizeof(buf));
		if (sta->addr != NULL) {
			snprintf(buf, sizeof(buf), MACSTR, MAC2STR(sta->addr));
		} else {
			snprintf(buf, sizeof(buf), MACSTR, 0,0,0,0,0,0);
		}
		snprintf(cmd, sizeof(cmd), "%s %s had authenticated", hapd->conf->iface, buf);
		notify_result(cmd);
#endif	// CONFIG_IBOX
		sta->flags |= WLAN_STA_AUTH;
	}
